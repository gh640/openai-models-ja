...
| 画像モデル | 画像生成や編集向けのモデル群。 |
| 動画モデル | 動画生成向けのモデル群。 |
| リアルタイムモデル・音声モデル | リアルタイム会話、音声入出力、文字起こし向けのモデル群。 |
| コーディングモデル | Codex 系を中心としたソフトウェア開発向けモデル群。 |
| ディープリサーチ | 深い調査タスク向けに設計されたモデル群。 |
| オープンウェイトモデル | Apache 2.0 ライセンスのオープンウェイトモデル。ローカル環境や独自デプロイ向け。 |
| ChatGPT モデル | ChatGPT 製品内で使われるモデル群。 API 利用は推奨されないものを含む。 |
| その他 | 上記の公式カテゴリに当てはまらない既存モデル。 |
...
OpenAI の最上位クラスの汎用モデル群。幅広いタスク向けに推奨されるモデルです。
...
- Reasoning: 最高（推論トークン対応、 `reasoning.effort=medium|high|xhigh` ）
- Speed: 最低速
...
最新の画像生成モデル。指示追従性とプロンプトへの忠実さが改善されています。
...
- Reasoning: 非常に高い（推論トークン対応、 `reasoning.effort=none` も設定可）
- Speed: 中
- 価格（1M トークンあたり）: 入力 $1.25 / キャッシュ入力 $0.125 / 出力 $10.00
- コンテキストウィンドウ: 400,000
- 最大出力トークン: 128,000
...
README に既に含まれているモデルのうち、上記の公式カテゴリ画像に含まれていないものをまとめています。

### GPT-5.2

GPT-5.4 の前世代にあたるフラグシップモデル。コーディングとエージェントタスク向けの高性能モデルです。
...
- Reasoning: 非常に高い（推論トークン対応、 `reasoning.effort=medium|high|xhigh` ）
- Speed: 低（シリーズ中最も遅い）
...
- 1.05M コンテキスト対応モデル（ GPT-5.4 / GPT-5.4 pro ）では、入力が 272K トークンを超えるとセッション全体で入力単価 2 倍・出力単価 1.5 倍が適用されます（標準 / Batch / Flex）。
- Regional processing（ data residency ）エンドポイントでは、 GPT-5.4 / GPT-5.4 pro に 10% の割増料金が適用されます。
