...
コーディングとエージェントタスク向けのフラグシップモデル。幅広い業務用途を想定した高性能モデルです。
...
- 入力モダリティ: テキスト・画像
- 出力モダリティ: テキスト
- 特徴: 「Featured models」でも紹介される代表モデル。コーディング支援とマルチステップのエージェント処理の両方に強い。
...
API リクエストに返答する前にマルチターンモデルインタラクションを行えるモデル。 Responses API のみで利用可。難しい問題に取り組むために設計されており、リクエスト処理に数分かかることがあります。タイムアウト回避にはバックグラウンドモードでの利用が推奨されます。 `reasoning.effort` として `medium` `hight` `xhigh` をサポートします。
